...
| [prompts.md](references/prompts.md) | Custom prompts at all four levels — workspace, series, occurrence, call. How the levels combine (series + occurrence BOTH apply), scopes, finding `eventId` / `seriesMasterId` via `/v1/calendar/events`, when each takes effect; versions of the AI outputs are per participant instruction set (workspace + series/occurrence + notes), never aggregated |
