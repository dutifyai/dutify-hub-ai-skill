...
Read [service-qualified catalog discovery](references/catalog.md) for the preferred versioned Hub discovery route and precise legacy fallback rules. Public catalog calls omit credentials. This skill remains independently usable.
...
- **Version:** `2026.09.25.2` — also in the frontmatter `version` and the root [`VERSION`](VERSION) file. Format is CalVer `YYYY.MM.DD`, with an optional `.N` suffix for a second release the same day.
...
- **Check for a newer version before version-sensitive work:**
  ...
  - *Flat install (no git):* compare your local `VERSION` to the repo's, then re-pull/reinstall if it's newer —
    ```
    curl -s https://raw.githubusercontent.com/dutifyai/dutify-hub-ai-skill/main/VERSION
    ```
- **Maintainers:** on every change ("pump"), bump the frontmatter `version` **and** the `VERSION` file in the same commit. A second change the same day → add/increment the `.N` suffix.
...
| [prompts.md](references/prompts.md) | Custom prompts at all four levels — workspace, series, occurrence, call. How the levels combine (series + occurrence BOTH apply), scopes, finding `eventId` / `seriesMasterId` via `/v1/calendar/events`, when each takes effect; versions of the AI outputs are per participant instruction set (workspace + series/occurrence + notes + note template), never aggregated |
